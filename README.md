...
```bash
go run . compress <input_image_path> <output_gac_path> [target_width|orig] [char_aspect_ratio] [color_scale]
```
...
#### Terminal Viewer (`view`)
```bash
go run . view <input_gac_path>
```

#### Reconstruct to PNG (`export`)
```bash
go run . export <input_gac_path> <output_image_path> [pixel|render]
```
...
- Run standard image tests: `task test`
- Run high-resolution image tests: `task test-highres`
